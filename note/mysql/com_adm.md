...
|中項目|小項目|
|:---|:---|
|[追加系コマンド](#追加系コマンド)|[テーブルやレコード、DBの作成、追加( CREATE, INSERT )](#テーブルやレコードのコピー)/ [テーブルやレコードのコピー( CREATE, INSERT )](#テーブルやレコードのコピー)|
|[キーの設定](#キーの設定)|主キー( primary key )/ ユニークキー( unique )/ 自動採番関連( auto_increment ) /フィールド初期値( default )|
|[確認系コマンド](#確認系コマンド)|( show, desc, select * from )|
|[改造系コマンド](#改造系コマンド)|[フィールドの値を変更( alter table )](#フィールドの値を変更)/  [フィールド位置の変更( alter table … first等 )](#フィールド位置の変更)|
|[削除系コマンド](#削除系コマンド)|DB,TABLE削除( drop ) /RECORD全削除 ( delete from )/ FIELD削除( alter table … drop …)|
|[レコード編集](#レコード編集)|[レコード追加( insert into ... value ... )](#レコード追加)/  [レコード変更( update ... set ...'' )](#レコード変更)/  [レコード削除( delete from ... where ... )](#レコード削除)|
...
|Ex|alter table TB名 change 元FIELD名 新FIELD名 新データ型|フィールドの名前とデータ型を変更||
...
||コマンド|役割|備考|
|:---|:---|:---|:---|
||insert into TB名 values(,入力値,入力値,…);|レコード一括挿入、全項目入力時のみFIELD名が不要||
||insert into TB名(FIELD1, FIELD3,FIELD5) values(FIELD1入力値, FIELD3入力値, FIELD5入力値);|指定したFIELDのみにレコードを挿入||
...
|Ex|update customer set birthday = '19750501' where birthday = '19650501';|customerテーブルのbirthdayフィールドのデータが19650501に該当するデータのみ19750501に変更する||
